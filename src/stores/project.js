...
export const useProjectStore = defineStore("project", () => {
  ...
  async function fetchSubCollections(projectId) {
    ...
    const [tasksSnap, docsSnap, calSnap, imgSnap, msgSnap] = await Promise.all([
  ...
  async function uploadProjectFile(file, subcollectionName) {
    ...
    try {
      ...
      const fileRef = storageRef(storage, `projects/${currentProject.value.id}/${subcollectionName}/${file.name}`)
      ...
      const docRef = await addDoc(collection(db, "projects", currentProject.value.id, subcollectionName), docData)

      // Pusher det til lokal Pinia state med det samme
      if (subcollectionName === "documents") documents.value.push({ id: docRef.id, ...docData })
      if (subcollectionName === "images") images.value.push({ id: docRef.id, ...docData })
  ...
  return {
    currentProject, loading, tasks, documents, calendar, images, messages,
    fetchProject, uploadProjectFile
  }
})